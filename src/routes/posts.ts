 ...
 * 发表文章 - 提交
 */
router.post('/', postValidator, (req: Request, res: Response, next: NextFunction) => {
  // TODO:
...
router.get('/:postId', (req: Request, res: Response, next: NextFunction) => {
  ...
  const queryPromises: Array<Promise<any>> = [
    ...
    Post.findOneAndUpdate({ _id: new ObjectId(postId) }, { $inc: { pv: 1 } }, { new: true })
      .populate('author', '_id username gender bio')
      .exec(),
    Comment.find({ post: new ObjectId(postId) })
      .populate('author')
      .exec(),
  ];
  ...
  Promise.all(
    queryPromises.map(
      (query: Promise<any>, index: number): Promise<any> => {
 ...
 * 编辑文章页面
 */
router.get('/:postId/edit', notLogin, (req: Request, res: Response, next: NextFunction) => {
  const postId: string = req.params.postId;
  const author: string = req.session.user._id;
 ...
 * 编辑文章页面 - 提交
 */
router.post('/:postId/edit', notLogin, postValidator, (req: Request, res: Response, next: NextFunction) => {
  const { title, content } = req.body;
  const author: string = req.session.user._id;
  const postId: string = req.params.postId;
...
router.get('/:postId/remove', notLogin, (req: Request, res: Response, next: NextFunction) => {
  const author: string = req.session.user._id;
  const postId: string = req.params.postId;
 ...
 * 提交留言
 */
router.post('/:postId/comment', notLogin, (req: Request, res: Response, next: NextFunction) => {
 ...
 * 删除留言
 */
router.get('/:postId/comment/:commentId/remove', notLogin, (req: Request, res: Response, next: NextFunction) => {
  const author: string = req.session.user._id;
